...
/-!
# Exact-rational Phase-1 simplex for LP feasibility over `Rat`
...
exist `λ ∈ Ratᵏ` with `λ ≥ 0`, `Σ λᵢ = 1`, `Σ λᵢ · mᵢ = 2·exp(m)`?

That's a small equality LP. We solve feasibility exactly over `Rat` via
...
private inductive StepResult where
  ...
  | progress (tab : Array (Array Rat)) (basis : Array Nat)
...
private def step (tab : Array (Array Rat)) (basis : Array Nat)
...
private inductive Phase1Outcome where
  | optimal (tab : Array (Array Rat)) (basis : Array Nat)
  | unbounded
...
exhaustion. -/
private def runPhase1 : Nat → Array (Array Rat) → Array Nat → Nat → Nat →
    Phase1Outcome
  | 0, _, _, _, _ => .exhausted
...
exactly over `Rat`, if one is found. `A` has `b.size` rows; rows of `A`
whose length differs from the common column count return `none`. -/
def findFeasibleEqLP? (A : Array (Array Rat)) (b : Array Rat) :
  ...
  let mut Ab : Array (Array Rat) := Array.mkEmpty rows
  let mut bs : Array Rat := Array.mkEmpty rows
  for i in [0:rows] do
    let row₀ := A[i]!
    if row₀.size ≠ nVars then return none
    if b[i]! < 0 then
      let mut neg : Array Rat := Array.mkEmpty nVars
  ...
  -- `-z` where `z = Σ b_i`.
  let mut cost : Array Rat := Array.replicate nCols 0
  for j in [0:nVars] do
    let mut s : Rat := 0
    for i in [0:rows] do
      s := s + Ab[i]![j]!
  ...
  let mut tab : Array (Array Rat) := Array.mkEmpty (rows + 1)
  tab := tab.push cost
  for i in [0:rows] do
    let mut row : Array Rat := Array.replicate nCols 0
    for j in [0:nVars] do
  ...
  | .optimal final finalBasis =>
    ...
    let mut sol : Array Rat := Array.replicate nVars 0
    for i in [0:rows] do
      let v := finalBasis[i]!
      if v < nVars then
...
`Rat`. `A` has `b.size` rows; rows of `A` whose length differs from the
common column count return `false`. -/
def isFeasibleEqLP (A : Array (Array Rat)) (b : Array Rat) : Bool :=
